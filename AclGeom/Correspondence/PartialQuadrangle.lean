...
/-- The full arrow family `X₀ ⟶ X₁` in the presented parameter groupoid,
based at the `T`-arrow with parameter `e`, carries the genuine difference-
chart group chunk.  Its operations are inherited from the vertex group,
so associativity and both inverse laws hold everywhere. -/
def parameterTArrowChunk (h : IsPartialQuadrangle f) (e : K) :
    RationalGroupChunk
      (PresentedFamilyGroupoid.x0 (parameterMultiplication h) ⟶
        PresentedFamilyGroupoid.x1 (parameterMultiplication h)) :=
  groupoidArrowChunk (parameterTArrow h e)

/-- The multiplication in the based parameter-arrow chunk is exactly the
groupoid difference product `a ≫ e⁻¹ ≫ b`. -/
theorem parameterTArrowChunk_mul (h : IsPartialQuadrangle f) (e a b : K) :
    (parameterTArrowChunk h e).mul
        (parameterTArrow h a) (parameterTArrow h b) =
      groupoidDifferenceProduct (parameterTArrow h e)
        (parameterTArrow h a) (parameterTArrow h b) :=
  rfl

/-- At four independent generic parameters, multiplication in the genuine
arrow chunk returns to the positive-dimensional `T`-parameter chart.  The
output parameter is supplied by the exact four-arrow family locus. -/
theorem exists_parameterTArrowChunk_mul [IsAlgClosed K]
    (h : IsPartialQuadrangle f) {s e a b : K}
    (hind : AlgebraicIndependent k ![s, e, a, b]) :
    ∃ c : K,
      (parameterTArrowChunk h e).mul
          (parameterTArrow h a) (parameterTArrow h b) =
        parameterTArrow h c := by
  obtain ⟨D, hD⟩ := h.exists_parameter_groupoidDifferenceProduct hind
  exact ⟨D.c, (parameterTArrowChunk_mul h e a b).trans hD.symm⟩
